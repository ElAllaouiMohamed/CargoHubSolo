...
def test_get_item_line_by_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_lines/{item_line_id}", headers={"API_KEY": api_key}
    )
...
def test_add_and_delete_item_line(api_data):
    ...
    post_response = requests.post(
        f"{url}/item_lines", json=new_item_line, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added item_line
    delete_response = requests.delete(
        f"{url}/item_lines/{new_item_line['id']}", headers={"API_KEY": api_key}
    )
...
def test_get_item_line_invalid_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_lines/{invalid_id}", headers={"API_KEY": api_key}
    )
...
def test_get_item_line_empty_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_lines/{empty_id}", headers={"API_KEY": api_key}
    )
...
def test_get_item_line_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_lines/{negative_id}", headers={"API_KEY": api_key}
    )
