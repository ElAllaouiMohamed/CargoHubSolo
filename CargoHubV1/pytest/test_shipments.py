...
def test_get_shipment_by_id(api_data):
    ...
    response = requests.get(
        f"{url}/shipments/{shipment_id}", headers={"API_KEY": api_key}
    )
...
def test_add_and_delete_shipment(api_data):
    ...
    post_response = requests.post(
        f"{url}/shipments", json=new_shipment, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added shipment
    delete_response = requests.delete(
        f"{url}/shipments/{new_shipment['id']}", headers={"API_KEY": api_key}
    )
...
def test_get_shipment_invalid_id(api_data):
    ...
    response = requests.get(
        f"{url}/shipments/{invalid_id}", headers={"API_KEY": api_key}
    )
...
def test_get_shipment_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/shipments/{negative_id}", headers={"API_KEY": api_key}
    )
