...
def test_get_location_by_id(api_data):
    ...
    response = requests.get(
        f"{url}/locations/{location_id}", headers={"API_KEY": api_key}
    )
...
def test_add_and_delete_location(api_data):
    ...
    post_response = requests.post(
        f"{url}/locations", json=new_location, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added location
    delete_response = requests.delete(
        f"{url}/locations/{new_location['id']}", headers={"API_KEY": api_key}
    )
...
def test_get_location_invalid_id(api_data):
    ...
    response = requests.get(
        f"{url}/locations/{invalid_id}", headers={"API_KEY": api_key}
    )
...
def test_get_location_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/locations/{negative_id}", headers={"API_KEY": api_key}
    )
