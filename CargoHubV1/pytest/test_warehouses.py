...
def test_get_warehouse_by_id(api_data):
    ...
    response = requests.get(
        f"{url}/warehouses/{warehouse_id}", headers={"API_KEY": api_key}
    )
...
def test_add_and_delete_warehouse(api_data):
    ...
    post_response = requests.post(
        f"{url}/warehouses", json=new_warehouse, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added warehouse
    delete_response = requests.delete(
        f"{url}/warehouses/{new_warehouse['id']}", headers={"API_KEY": api_key}
    )
...
def test_get_warehouse_invalid_id(api_data):
    ...
    response = requests.get(
        f"{url}/warehouses/{invalid_id}", headers={"API_KEY": api_key}
    )
...
def test_get_warehouse_empty_id(api_data):
    ...
    response = requests.get(
        f"{url}/warehouses/{empty_id}", headers={"API_KEY": api_key}
    )
...
def test_get_warehouse_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/warehouses/{negative_id}", headers={"API_KEY": api_key}
    )
