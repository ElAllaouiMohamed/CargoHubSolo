...
def test_get_item_type_by_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_types/{item_type_id}", headers={"API_KEY": api_key}
    )
...
def test_add_and_delete_item_type(api_data):
    ...
    post_response = requests.post(
        f"{url}/item_types", json=new_item_type, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added item_type
    delete_response = requests.delete(
        f"{url}/item_types/{new_item_type['id']}", headers={"API_KEY": api_key}
    )
...
def test_get_item_type_invalid_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_types/{invalid_id}", headers={"API_KEY": api_key}
    )
...
def test_get_item_type_empty_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_types/{empty_id}", headers={"API_KEY": api_key}
    )
...
def test_get_item_type_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/item_types/{negative_id}", headers={"API_KEY": api_key}
    )
