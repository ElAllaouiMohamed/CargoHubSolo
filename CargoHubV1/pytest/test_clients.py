...
def test_add_and_delete_client(api_data):
    ...
    post_response = requests.post(
        f"{url}/clients", json=new_client, headers={"API_KEY": api_key}
    )
    assert post_response.status_code == 201

    # Delete the newly added client
    delete_response = requests.delete(
        f"{url}/clients/{new_client['id']}", headers={"API_KEY": api_key}
    )
...
def test_update_client(api_data):
    ...
    original_data = requests.get(
        f"{url}/clients/{client_id}", headers={"API_KEY": api_key}
    ).json()
    ...
    put_response = requests.put(
        f"{url}/clients/{client_id}", json=updated_client, headers={"API_KEY": api_key}
    )
    ...
    restore_response = requests.put(
        f"{url}/clients/{client_id}", json=original_data, headers={"API_KEY": api_key}
    )
...
def test_get_client_negative_id(api_data):
    ...
    response = requests.get(
        f"{url}/clients/{negative_id}", headers={"API_KEY": api_key}
    )
