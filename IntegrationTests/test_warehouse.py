...
class TestWarehousesEndpoint(unittest.TestCase):
    ...
    def test_1_post_warehouse(self):
        ...
        self.assertIn(response.status_code, (200, 201), f"Failed to create warehouse: {response.text}")
        data = response.json()
        ...
        self.assertEqual(data["contact"]["name"], self.test_warehouse["contact"]["name"])
    ...
    def test_2_get_warehouse(self):
        ...
        self.assertEqual(response.status_code, 200, f"Failed to get warehouse: {response.text}")
        data = response.json()
    ...
    def test_3_update_warehouse(self):
        ...
        response = self.client.put(f"{self.base_url}{self.test_id}", json=self.updated_warehouse)
        self.assertEqual(response.status_code, 200, f"Failed to update warehouse: {response.text}")
        data = response.json()
        self.assertEqual(data["name"], self.updated_warehouse["name"])
        self.assertEqual(data["code"], self.updated_warehouse["code"])
        self.assertEqual(data["contact"]["name"], self.updated_warehouse["contact"]["name"])
        self.assertFalse(data["isDeleted"])

    def test_4_get_all_warehouses(self):
        response = self.client.get(self.base_url)
        self.assertEqual(response.status_code, 200, f"Failed to get all warehouses: {response.text}")
        data = response.json()
    ...
    def test_5_soft_delete_warehouse(self):
        ...
        self.assertEqual(response.status_code, 204, f"Failed to soft delete warehouse: {response.text}")

        get_response = self.client.get(f"{self.base_url}{self.test_id}")
        self.assertEqual(get_response.status_code, 404, "Soft-deleted warehouse should return 404")
