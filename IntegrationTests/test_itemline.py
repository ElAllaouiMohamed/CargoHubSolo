...
class TestItemLineEndpoint(unittest.TestCase):
    ...
    def test_1_post_itemline(self):
        ...
        self.assertEqual(data.get("name") or data.get("Name"), self.test_itemline["name"])
    ...
    def test_3_update_itemline(self):
        ...
        response = self.client.put(f"{self.base_url}{self.created_itemline_id}", json=self.updated_itemline)
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data.get("description") or data.get("Description"), self.updated_itemline["description"])
