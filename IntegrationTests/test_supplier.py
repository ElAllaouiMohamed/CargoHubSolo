...
class TestSuppliersEndpoint(unittest.TestCase):
    ...
    def test_1_post_supplier(self):
        ...
        self.assertIn(response.status_code, (200, 201), f"Failed to create supplier: {response.text}")
        data = response.json()
    ...
    def test_4_get_all_suppliers(self):
        response = self.client.get(self.base_url)
        self.assertEqual(response.status_code, 200, f"Failed to get all suppliers: {response.text}")
        data = response.json()
